...
public class LinkHashTableImpl<K, V> implements LinkedHashTable<K, V> {
    //private final Item<K, V> [] data;//пример с урока
...
//    public HashTableImpl(int initialCapacity) {//пример с урока
//        this.data = new Item[initialCapacity];
//    }

    public LinkHashTableImpl() {
        this(16);
    }

    public LinkHashTableImpl(int initialCapacity) {
    ...
    private int getStepLinear() { //линейное пробитие
        return 1;
    }

    private boolean isKeyEquals(ItemLink<K,V> item, K key) {
